...
gst_d3d11_compositor_pad_class_init (GstD3D11CompositorPadClass * klass)
{
  ...
  g_object_class_install_property(object_class,
    PROP_PAD_CROP, g_param_spec_boxed("crop", "crop properties",
      "provide left,right,top,bottom coordinates",
      GST_TYPE_STRUCTURE, param_flags));
...
gst_d3d11_compositor_pad_init (GstD3D11CompositorPad * pad)
{
  ...
  pad->crop_x = DEFAULT_PAD_XPOS;
  pad->crop_y = DEFAULT_PAD_YPOS;
  pad->crop_width = DEFAULT_PAD_WIDTH;
  pad->crop_height = DEFAULT_PAD_HEIGHT;
...
gst_d3d11_compositor_pad_dispose (GObject * object)
{
  ...
  GstD3D11Device *device = (GstD3D11Device *)gst_object_ref (self->convert->device);
  
  gst_clear_object (&self->convert);

  if (self->blend) {
    GstD3D11DeviceLockGuard lk (device);
    gst_d3d11_device_fence_simple (device);
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_PAD_CROP:
    {
      ...
      pad->crop_properties = gst_structure_copy(gst_value_get_structure(value));
      ...
      if (pad->convert != NULL)
      {
        ...
        pad->crop_x = left;
        pad->crop_y = top;
        pad->crop_width = right - left;
        pad->crop_height = bottom - top;
        pad->apply_crop = TRUE;
      }
      break;
    }
